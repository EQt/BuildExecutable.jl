...
function build_executable(exename, script_file, targetdir=nothing, cpu_target="native";
    ...
    if force
        ...
        for f in [userimgjl, "$(sys.buildfile).$(Libdl.dlext)", "$(sys.buildfile).ji", exe_file.buildfile]
            if isfile(f)
    ...
    cmd = `gcc $win_arg -shared $(sys.buildfile).o -o $(sys.buildfile).$(Libdl.dlext) -L$(sys.libjulia) $(exe_file.libjulia) $gcc_args`
    info(cmd)
    run(cmd)
